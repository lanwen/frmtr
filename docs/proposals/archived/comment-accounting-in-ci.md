> **Status: Implemented.** Landed on `main`: the `FormatterGuardrails.STRICT_CLAIMS_PROPERTY` guardrail split and the asserting output-level `CommentPresenceDiagnosticTest` lexer net (strict-claims is now `true` in `frmtr-core/build.gradle.kts`). Archived 2026-07-14; retained as a provenance record. See [Outcome](#outcome).
