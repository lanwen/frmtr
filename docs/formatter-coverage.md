...
| `AnnotationDeclaration` | `AnnotationDeclarationPrinter` | Owns annotation type headers; recovered annotation-member gaps stay inside this printer. Both empty and populated member blocks (outside parse-error recovery) delegate to `MemberBlockPrinter` so brace-line and orphan comments are preserved like other type bodies instead of being dropped, while `MemberBlockPrinter` keeps the annotation body's single-line gap after the opening brace. |
...
| `ReturnStmt` | `StatementPrinter`, `ReturnExpressionPrinter`, and `ReturnBinaryExpressionLayout` | `StatementPrinter` owns statement assembly; `ReturnExpressionPrinter` owns return-value wrapping and delegates direct binary returns to `ReturnBinaryExpressionLayout`. Return constructor source-shape policy comes from `ObjectCreationLayoutPolicy`. |
...
| `IfStmt` | `StatementPrinter`, `ControlConditionPrinter`, and `ControlConditionMethodCallLayout` | `StatementPrinter` owns if/else chain structure, empty branches, and between-branch comments; `ControlConditionPrinter` owns parenthesized condition layout including source-multiline binary condition breaks, with method-call operand and control-context width policy delegated to `ControlConditionMethodCallLayout`. Nested statements return through statement dispatch. |
| `WhileStmt` | `StatementPrinter` | Conditions route through `ControlConditionPrinter` and `ControlConditionMethodCallLayout`; bodies route through statement/block owners. |
| `DoStmt` | `StatementPrinter` | Conditions route through `ControlConditionPrinter` and `ControlConditionMethodCallLayout`; bodies route through statement/block owners. |
...
| `SynchronizedStmt` | `StatementPrinter` | Conditions route through `ControlConditionPrinter` and `ControlConditionMethodCallLayout`; body routes to `BlockPrinter`. |
...
- `ControlConditionPrinter` owns parenthesized conditions for `if`, loops, synchronized statements, and statement-switch
  ...
  condition breaks and method-call operand policy delegated to `ControlConditionMethodCallLayout`; switch expressions use
  compact selector text directly.
...
| `ReturnBinaryExpressionLayout` | Direct binary return layout, including method-call-left continuations and return-specific decisions that leave nested source-multiline method-call arguments with ordinary expression rendering. |
...
| `LambdaParameterHeaderLayout` | Canonical lambda parameter and header rendering before `->`, including parenthesis policy, commented parameter reconstruction, header width breaks, and source-multiline parameter detection. |
| `ExpressionLambdaClosingLayout` | Source-shaped call-closing placement for source-multiline expression-lambda arguments with simple logical bodies. |
| `PackedLambdaBody` | Small value boundary carrying a packed expression-lambda body with the selected call-closing suffix placement. |
| `SourceMultilineLambdaCallLayout` | Shared preservation for source-multiline expression-lambda method-call bodies, method-chain first-segment attachment, and block-lambda parameter lists that were multiline in source. Callers keep ownership of root promotion, chain suffixes, object-construction policy, and statement syntax. |
| `TextBlockArgumentSourceLayout` | Source indentation recovery for text-block arguments inside expression-lambda method-call bodies. It does not own literal content rendering, ordinary call closure, or comment eligibility decisions. |
