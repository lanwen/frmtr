class MethodCallArgumentContextWidth {
    ...
    void recordBranch(DocList docs, ShipmentBranch shipmentBranch, ShipmentRenderer shipmentRenderer) {
        docs.add(
            shipmentBranch.isFallbackRoute()
                ? shipmentRenderer.format(shipmentBranch)
                : nestedShipmentBranch(shipmentBranch)
        );
    }
    ...
    void recordGroupedBranch() {
        Doc doc = Doc.group(
            Doc.concat(Doc.text("prefix"), Doc.ifBreak(Doc.text("-broken-branch"), Doc.text("-flat")))
        );
        sink(doc);
    }
