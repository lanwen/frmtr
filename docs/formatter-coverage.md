...
| `AnnotationDeclaration` | `AnnotationDeclarationPrinter` | Owns annotation type headers and non-empty member blocks; recovered annotation-member gaps stay inside this printer. Empty member lists delegate to `MemberBlockPrinter` so a comment-only empty body preserves its comment like other type bodies instead of collapsing to `{}`. |
