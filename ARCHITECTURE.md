...
- The adopted upstream `prettier-java` fixture set under `frmtr-core/src/test/resources/format/prettier-java`, with parseable inputs, parseable Prettier Java reference outputs, checked-in `frmtr.output.java` snapshots for current formatter output, and an explicit Prettier-compatible subset compared directly against `prettier.output.java` using Prettier-style 80-column, two-space options, raw trailing-whitespace preservation for ignored regions, and require-pragma mode for require-pragma fixtures. The preserved upstream mirror under `frmtr-core/src/test/resources/upstream/prettier-java` uses the same `input.java` and `prettier.output.java` file names so fixture pairs are named consistently across both trees.
