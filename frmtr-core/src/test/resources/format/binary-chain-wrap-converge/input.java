class BinaryChainWrapConverge {
    boolean binaryFlatSource(RemoteRef rx, LocalRef item) {
        ...
        if (item.ready()) {
            if (item.tracked()) {
                if (item.fastLookup()) {
                    found = rx.fileLabelName().equals(item.absoluteItemPath()) || rx.fileLabelName().equals(item.shortLabelName());
                } else {
    ...
    void chainFlatSource(SessionStore sessionStore) {
        sessionProvider = sessionKey -> SlidingWindowSessionCache.builder().key(sessionKey).maxEntries(10).sessionStore(sessionStore).build();
    }
