public class FrameRouter {
  ...
  enum RequestKind {
    PRODUCE, FETCH, LIST_OFFSETS, METADATA, OFFSET_COMMIT, OFFSET_FETCH, FIND_COORDINATOR, JOIN_GROUP,
    DESCRIBE_GROUPS, LIST_GROUPS, SASL_HANDSHAKE, API_VERSIONS, CREATE_TOPICS, DELETE_TOPICS, HEARTBEAT
  }

  RoutingDecision route(LinkState linkState, RequestKind requestKind, RequestContext request, FrameRegistry registry) {
    return switch (linkState) {
      case ESTABLISHED -> {
        yield switch (requestKind) {
          case PRODUCE, FETCH, LIST_OFFSETS, METADATA, OFFSET_COMMIT, OFFSET_FETCH, FIND_COORDINATOR, JOIN_GROUP -> {
            yield RoutingDecision.forwarded(requestKind);
          }
          case DESCRIBE_GROUPS, LIST_GROUPS, SASL_HANDSHAKE, API_VERSIONS, CREATE_TOPICS, DELETE_TOPICS -> registry.dispatch(request);
