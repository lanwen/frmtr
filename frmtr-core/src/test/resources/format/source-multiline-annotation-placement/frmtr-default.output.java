...
class SourceMultilineAnnotationPlacement {
    ...
    record RouteSnapshot(
        @RouteKey(
            value = "primary-zone-audit-index-that-is-intentionally-long-for-formatter-pass-coverage-and-record-placement"
        ) String routeKey,
        Map<
            @EncodedKeys(
    ...
    @PlanNames({
        "northbound-scheduler-window",
        "southbound-scheduler-window",
        "overnight-backfill-window",
    ...
    @AuditLabel(
        "ticket-"
            + Labels.ACTIVE_REGION
            + "-catalog-"
            + Labels.ARCHIVE_REGION
            + "-fallback-"
            + Labels.REPLAY_REGION
    )
    void refreshCatalog() {}
}
