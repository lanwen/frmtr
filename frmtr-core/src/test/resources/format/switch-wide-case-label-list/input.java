public class OrderEventRouter {
  ...
  OrderStatus route(OrderEvent event) {
    return switch (event) {
      case PaymentAuthorized _, ShipmentDispatched _, CustomerCancellationRequested _, RefundSettlementCompleted _, ChargebackDisputeOpened _ -> {
        yield OrderStatus.progressed();
      }
      case InventoryReserved _, FraudReviewRequested _, BackorderThresholdExceeded _, SupplierRestockConfirmed _, WarehouseTransferInitiated _ -> OrderStatus.unchanged();
