...
| **LDM-2** | **Measurement parity (C10)** | Chain/initializer width measured at the real output column (`DocWidths`) instead of `LayoutWidth`/source-column baselines; verified-rebaseline affected goldens. On the chain root path the grouped-promoted multi-argument root offers its grouped and fully-broken shapes to `Doc.bestFitting`, so the renderer ranks them at the true rendered column rather than a source estimate; the single-segment method-root break is written in the same `bestFitting` form, but its sole caller passes a scope-less method root for which no broken alternative is offered, so that ranking stays inert. `LayoutWidth.nodeLine` still backs the remaining chain-path estimate consumers, and retiring them is out of scope here: three shape-emitting gates in `ChainRootPromotionLayout` keep the estimate because their hard-break lambda bodies defeat fewest-lines ranking — the block-lambda multi-argument pre-emption and both block-lambda root hugs (the grouped-promoted hug and the single-segment promoted-root hug), where an over-width opener and an over-width body line would both fail `fits()` and let the tie-break pick the over-width hug. In `MethodCallChainPrinter` the surviving reads are two routing bailouts (guards that withhold a path via `Optional.empty`, not shape emissions), a first-line width-string producer that feeds a width probe rather than a `Doc`, and a pair of reads behind `canAttachFirstSegmentToSimpleRoot`, which the predicate never satisfies so they are currently dead. | rebaseline (reviewed) | LDM-1 |
