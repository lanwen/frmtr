...
final class OngoingReassignmentCollector {
    ...
    void collectPartitions(
            List<OngoingTopicReassignment> ongoingTopicReassignments,
            ListPartitionReassignmentsRequestData data
    ) {
        if (data.topics() != null) {
                    ...
                    new OngoingTopicReassignment()
                            ...
                            .setPartitions(
                                topic.partitionIndexes()
                                        .stream()
                                        .map(partitionIndex -> new OngoingPartitionReassignment().setPartitionIndex(
                                            partitionIndex
                                        ))
