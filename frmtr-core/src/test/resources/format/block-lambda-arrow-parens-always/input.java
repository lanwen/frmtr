public class Lambda {
  ...
  public void shortLambdaAssignation() {
    ...
    void huggableArguments() {
        ...
        largeCustomerBatch((billingAccountRecord, customerSegmentRecord, deliveryWindowRecord) -> eligibilityRuleSet.calculateDiscounts());

        a.b(c -> d -> eligibility.compute(regionalInventory, shipmentWindows, accountStatus, pricingRules, taxRules));

        a.b(c -> d && eligibility.compute() ? g && shipmentWindows.accountStatus() : j && taxRules.hasOverride());

        a.b(c -> d && eligibility.compute(regionalInventory, shipmentWindows, accountStatus, pricingRules, taxRules) > 0);

        a.b(c, (c0, c1) -> d && eligibility.compute(regionalInventory, shipmentWindows, accountStatus, pricingRules, taxRules) > 0);

        a.b(c -> eligibility.compute(regionalInventory, shipmentWindows, accountStatus, pricingRules, taxRules) > 0);

        a.b(c, (c0, c1) -> eligibility.compute(regionalInventory, shipmentWindows, accountStatus, pricingRules, taxRules) > 0);

        a.b(c -> d && eligibility.compute(regionalInventory, shipmentWindows, accountStatus, pricingRules, taxRules));

        a.b(c, (c0, c1) -> d && eligibility.compute(regionalInventory, shipmentWindows, accountStatus, pricingRules, taxRules));
        ...
        a.b((c0, c1) -> eligibility.compute(regionalInventory, shipmentWindows, accountStatus, pricingRules, taxRules));

        a.b(c, (c0, c1) -> eligibility.compute(regionalInventory, shipmentWindows, accountStatus, pricingRules, taxRules));
        ...
        largeCustomerBatch((billingAccountRecord, customerSegmentRecord, deliveryWindowRecord
        // comment
        ) -> eligibilityRuleSet.calculateDiscounts());

        largeCustomerBatch( /* comment */
        (billingAccountRecord, customerSegmentRecord, deliveryWindowRecord) -> eligibilityRuleSet.calculateDiscounts());

        largeCustomerBatch( /* comment */ (billingAccountRecord, customerSegmentRecord, deliveryWindowRecord) -> eligibilityRuleSet.calculateDiscounts());
    ...
    void lambdaInParentheses() {
        (dispatchJob -> orderEvent.validateOrder().deliveryPlan().eligibility().compute());
    }
}
...
class T {
    ...
    T() {
        super((veryLongCustomerFilterParameter,
                  veryLongCustomerFilterParameter,
                  veryLongCustomerFilterParameter,
                  veryLongCustomerFilterParameter,
                  veryLongCustomerFilterParameter,
...
enum Enum {
    ...
    VALUE((veryLongCustomerFilterParameter,
              veryLongCustomerFilterParameter,
              veryLongCustomerFilterParameter,
              veryLongCustomerFilterParameter,
              veryLongCustomerFilterParameter,
