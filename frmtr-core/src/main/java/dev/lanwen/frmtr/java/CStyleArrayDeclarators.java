 ...
 * after the variable name) is rendered as valid Java without duplicating the variable name.
 *
 * <p>JavaParser models such a declarator as a {@link VariableDeclarator} whose {@link VariableDeclarator#getType()} is
 * an {@link ArrayType} built from {@link ArrayType.Origin#NAME} bracket pairs. Those bracket tokens sit after the
 * declarator name in source, so the type node's token range spans the name. Declaration printers build one shared type
 * prefix from the first declarator and then append each bare declarator name, which is why a naive token-range type
 * prefix re-emits the name and produces non-compiling {@code String filters[] filters} output.
 *
