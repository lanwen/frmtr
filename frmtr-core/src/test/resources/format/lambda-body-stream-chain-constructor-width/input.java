...
class ShardPlacementView {
    ...
    private static List<ShardPlacementReplica> readyReplicas(Map<ShardPartition, List<WorkerNode>> placement) {
        return placement.entrySet().stream().flatMap(entry -> entry.getValue().stream().filter(node -> !node.isDraining()).map(node -> new ShardPlacementReplica(entry.getKey().topic(), entry.getKey().partition(), node.id()))).toList();
    }
}
