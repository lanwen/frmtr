class MultilineIfConditionSample {
    ...
    void nestedRouteCheckpoint(Object checkpoint, WidthProbe widthProbe, RenderOptions options) {
        if (
            ...
            && (routeCheckpoint.sourceSpansMultipleStops()
                || widthProbe.measure(routeCheckpoint.compactRouteName()) > options.lineWidth())
        ) {
            sink.record(routeCheckpoint);
        }
    }

    void rejectOverwideRoute(
        ...
        if (
            routeBudget.estimatedTransferWidth(deliveryRoute.primaryStop(), routeLeg.compactSegmentName())
                > options.lineWidth()
        ) {
