public class SettlementRouter {
    ...
    static class ReconciliationStage {
        ...
        void route(SettlementSignal signal) {
            switch (signal) {
                case PaymentSettlementEvent settlementEvent when (
                    settlementEvent.reconciledAmount() > limitAmt
                ) -> handle();
                case ReconciliationBatch(PaymentSettlementEvent settlementEvent) when (
                    settlementEvent.reconciledAmount() > outstandingLimitAmount
                ) -> reconcile();
                default -> ignore();
            }
        }
