 ...
 *
 * <p>Every role is populated from the record-only dry-run pre-pass (see {@link CommentTracker#beginRecording}): the
 * dry-run records each comment's first claimant in print-traversal order under the slot the offering wrapper names, and
 * the real render consults {@link CommentTracker#ownsHere} for every role, so a comment renders only from the slot
 * recorded as its first claimant. The dry-run records the <em>real</em> traversal owner rather than a source-order
 * approximation because a pure source-order rule diverges on the contested leading/own families (the
 * parent-interleaver-beats-child cases) while matching on trailing.
 *
