public class BinaryOperations {
    ...
    public void method() {
        new ProfileRequest(profileId, accountId, "customer profile payload with long details", "more details").submit(10);
    ...
    void parentheses() {
        ...
        if (customerSubtotal + shippingFee == invoiceSubtotal + handlingFee && discountTotal + taxEstimate == paymentTotal + refundTotal || requiresReview) {
        }
    ...
    void instanceOf() {
        var hasSnapshot = hasSnapshot && AccountStore.find(cachedCustomerRecordIdentifier) instanceof CustomerAccountSnapshot snapshot && snapshot.isOpen();

        var snapshotMatch = AccountStore.find(cachedCustomerRecordIdentifier) instanceof CustomerAccountSnapshot snapshot && snapshot.isOpen();
