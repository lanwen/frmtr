class MethodChainTrailingLambdaComment {
    void verifySignals(Source source) {
        Probe.create(source)
            ...
            .thenConsumeWhile(items -> items.stream().allMatch(candidate -> candidate.remoteProviderState() == ExternalProviderState.TRANSITIONING)) // keep polling while external state settles
            .assertNext(items -> {
