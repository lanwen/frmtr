class SourceMultilineControlConditionSample {
    ...
    void selectIf() {
        ...
        if (segmentPlan.hasOpenSegment() || routePolicy.accepts(routeContext.primaryStop(), routeContext.backupStop(), segmentPlan.candidateWindow(), dispatchWindow.retryBudget())) {
            segmentPlan.reserve();
        }
    }

    void drainWhile() {
        while (routePolicy.accepts(routeContext.primaryStop(), routeContext.backupStop(), segmentPlan.candidateWindow(), dispatchWindow.retryBudget()) && segmentPlan.hasOpenSegment()) {
            segmentPlan.advance();
        }

        while (segmentPlan.hasOpenSegment() || routePolicy.accepts(routeContext.primaryStop(), routeContext.backupStop(), segmentPlan.candidateWindow(), dispatchWindow.retryBudget())) {
            segmentPlan.advance();
        }
    }
