...
final class LambdaExpressionArgumentOpener {
    ...
    FlowResult details(DispatchRepository dispatchRepository, ImageCounter imageCounter, Clock clock, String requestId) {
        return dispatchRepository
            .findByIdWithOwner(requestId)
            .flatMap(record ->
                imageCounter.count(record).map(counts -> DispatchDetails.from(clock, record, counts))
            );
    }
    ...
    FlowResult imageCounts(CacheTemplate cacheTemplate, String imagesKey) {
        return cacheTemplate
            ...
            .collectMap(Map.Entry::getKey, entry ->
                Long.parseLong(new String(entry.getValue(), StandardCharsets.UTF_8))
            );
    }
    ...
    StubFlow answerWithRepositoryCall(StubSource stubSource, BundleGateway regionalWindowBundleReadGateway) {
        return when(
            ...
            regionalWindowBundleReadGateway.findFirstLaunchBundlesForWindowTickets(
    ...
    StubFlow recoversDuplicateMember(MemberRepository memberRepository, Member member, User user, Org org) {
        return memberRepository
            ...
            .onErrorResume(DuplicateKeyException.class, ex ->
                memberRepository.findByUserIdAndOrganizationIdWithSnapshot(user.getId(), org.getId())
            );
    }
    ...
    StubFlow answerWithLongBodySelector(StubSource stubSource, BundleGateway regionalWindowBundleReadGateway) {
        return when(
            ...
            regionalWindowBundleReadGateway.findFirstLaunchBundlesForWindowTicketsWithVerifiedProjectionState(
                invocation.getArgument(0),
                invocation.getArgument(1)
            )
        );
    ...
    StepProbe explodesTrailingCallArgumentsWhenGluedLineOverflows(
        ...
        return probe.withVirtualTime(() ->
            new SessionReader(
                ...
                agentLedger,
                directoryClient
            ).findSessionsAcrossEveryRegisteredDownstreamConsumerGroupInTheDeployment(principal.groupId(), Source.REMOTE, principal, null)
        )
            .expectSubscription();
    }
    ...
    StepProbe keepsMethodCallLambdaBodyPacked(StepProbe probe, SessionReader sessionReader, Principal principal) {
        return probe.withVirtualTime(() ->
            sessionReader.findSessions(principal.groupId(), Source.LOCAL, principal, null)
        )
            .expectSubscription();
    }
                ...
                accountingWindows
                    ...
                    .map(accountingWindow -> {
                        return projectedRows
                            .stream()
                            .filter(projectedWindowUsage -> projectedWindowUsage.accountingWindow().equals(accountingWindow))
                            .findFirst()
                            .orElseGet(() ->
                                ProjectedWindowUsageSnapshot.builder()
                                    .tenantId(tenantId)
    ...
    AuditTrail keepsFlatScopedCallBodyStable(AuditTrail auditTrail, Segment segment, Defaults defaults) {
        return auditTrail.record(segment.primaryKey(), segment.fallbackKey(), entry -> entry.routingContext().resolvedPolicy().composeWindowSelection(defaults.policyGroup(), entry.lastVisibleWindow(), segment.ownerGroup(), defaults.clock()));
    }

    MatchPlan keepsFlatLogicalLambdaBodyStable(MatchPlan plan, Boundary boundary, Event event) {
        return plan.select(event.kind(), row -> row.owner().equals(event.owner()) && row.createdAt().isAfter(boundary.openedAt()) || row.priority() == Priority.FALLBACK && row.createdAt().isBefore(boundary.closedAt()));
    }

    DraftPlan keepsFlatMethodCallArgumentBodyStable(DraftPlan plan, Window window, Cursor cursor, String label) {
        return plan.map(window.id(), cursor.next(), slot -> buildProjectionEnvelope(slot.currentVersion(), window.ownerKey(), cursor.traceToken(), label, slot.expiryPolicy()));
    }

    RoutePlan keepsNestedLambdaArgumentBodyStable(RoutePlan plan, MarkerIndex markerIndex, String routeKey) {
        return plan.rewrite(routeKey, candidate -> markerIndex.find(candidate.ownerKey(), marker -> marker.visibleTo(candidate.viewer()) && marker.routeKey().equals(routeKey), candidate.defaultMarker()));
    }
}
