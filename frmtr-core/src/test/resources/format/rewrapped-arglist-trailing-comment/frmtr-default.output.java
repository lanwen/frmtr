...
class StateCleanup {
    ...
    void deleteState(String topologyName) {
        throw new IllegalStateException(
            "Unable to delete state for the named topology " + topologyName,
            new RuntimeException(topologyName)
        ); // use dummy taskid for this error
    }
    ...
    void deleteStateWidthDriven(String topologyName) {
        throw new IllegalStateException(
            "Unable to delete state for the named topology " + topologyName,
            new RuntimeException(topologyName)
        ); // width-driven re-wrap keeps the note
    }

    void reportCleanup(String topologyName, EventSink sink) {
        sink.recordCleanupFailure(
            "Unable to delete state for the named topology " + topologyName,
            new RuntimeException(topologyName)
        ); // call re-wrap keeps the note
    }
