class ReturnChainMultiSegmentRankedSample {
    ...
    String describeActiveProfile(DeploymentTarget deploymentTarget) {
        return deploymentTarget.getConfigurationRegistry().resolveActiveProfileSelector().formatActiveProfileLabelForDisplay();
    }

    RetryController buildRetryController(RequestPipeline requestPipeline) {
        return PipelineFactory.wrap(requestPipeline).withFailureRecovery(RetryPolicy.restart().withResetChildrenEnabled(true));
    }
