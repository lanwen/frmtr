...
final class FormatterFailureRendererTest {
    ...
    void rendersFormatterFailureAsStructuredDiagnosticText() {
        ...
        assertThat(diagnostic.lines().get(0).spans()).containsExactly(
            new DiagnosticSpan("Unable to parse Java source:", DiagnosticStyle.ERROR_TEXT)
        );
        assertThat(diagnostic.lines()).anySatisfy(line -> assertThat(line.spans()).containsExactly(
        ...
        assertThat(diagnostic.lines().get(diagnostic.lines().size() - 1).spans()).containsExactly(
            new DiagnosticSpan("(line 8,col 20) Parse error", DiagnosticStyle.ERROR_TEXT)
        );
    }
}
