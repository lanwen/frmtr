...
class ConnectionPoolCoordinator {
    ...
    void drainAvailableConnections() {
        while (!pendingConnectionRequests.isEmpty())
            connectionPool.acquireAvailableSlot(pendingConnectionRequests.poll());
    }
    ...
    void recordRetryOutcomes() {
        for (int attempt = 0; attempt < maxRetryAttempts; attempt++)
            connectionRetryBudget.recordAttemptOutcome(attempt);
    }
}
